  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={-1}
        className={cn(
          "bg-white rounded-2xl shadow-2xl w-full overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[90vh] outline-none animate-in zoom-in-95 duration-200",
          maxWidth,
          className
        )}
      >
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-gray-100">
            {showCloseButton ? (
              <button
                ...
                className="p-1.5 sm:p-2 -ml-1 sm:-ml-2 rounded-full hover:bg-gray-100 text-gray-700 transition"
              >
                <X className="w-5 h-5" />
              </button>
            ) : <div />}
            {title && <h2 className="text-sm sm:text-base font-semibold text-gray-900 truncate px-2">{title}</h2>}
            <div className="w-8" />
          </div>
        )}
