...
export function ReviewsSection({ ratings, reviews }) {
  ...
  return (
    <section id="reviews" className="py-8 border-b border-gray-200" aria-label="Guest reviews">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8 pb-6 border-b border-gray-100">
        {categories.map((cat) => (
          <div key={cat.label} className="border-r border-gray-200 last:border-r-0 pr-3">
            <div className="text-xs text-gray-500 font-medium mb-1">{cat.label}</div>
            <div className="text-base font-bold text-gray-900">{cat.score.toFixed(1)}</div>
          </div>
        ))}
      </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-4">
          {reviews.map((r) => (
            <div key={r.id} className="space-y-3 border-b border-gray-100 pb-6">
              <div className="flex items-center gap-3">
                <img
                  src={r.avatar}
                  alt={r.author}
              ...
              <p className="text-sm text-gray-800 leading-relaxed">{r.comment}</p>
            </div>
          ))}
        </div>
      </Modal>
